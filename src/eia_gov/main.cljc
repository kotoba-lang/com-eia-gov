(ns eia_gov.main
  "Kotodama WASM entrypoint for the eia_gov clean-room actor (L4) — Clojure port.
  ...
  facts (`eia_gov.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "eia_gov")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"eia_gov.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "eia_gov-compat" :tier tier :entities entities} 200])
...
(defn start! [] :eia_gov-compat/ready)
